...
import ReadingPicks from "@/components/reading/ReadingPicks";
import { getBookOfTheDay, getFinishMe, getWhatToReadNext } from "@/lib/recommendations";
...
export default async function Home() {
  ...
  const nextCandidates = books.filter((book) => getStatus(book) !== "Read");
  const initialNext = getWhatToReadNext(books);
  ...
  return (
    <AppShell>
      ...
      <div className="stats">
        ...
        <div className="stat"><div className="stat-label">COMPLETED</div><div className="stat-value">{completed}</div><div className="stat-small">{books.length ? Math.round((completed / books.length) * 100) : 0}% of your books</div></div>
        <div className="stat"><div className="stat-label">PAGES READ</div><div className="stat-value">{pagesRead.toLocaleString()}</div><div className="stat-small">across your library</div></div>
      ...
      <ReadingPicks
        ...
        bookOfDay={getBookOfTheDay(books)}
        finishMe={getFinishMe(books)}
            ...
            {continueReading.map((book) => (
              <Link href={`/book/${book.id}`} className="book-card" key={book.id}>
                <div className="book-cover">
                  {book.cover ? <OptimizedCover src={book.cover} sizes="(max-width: 700px) 45vw, (max-width: 1100px) 20vw, 180px" /> : <div className="cover-fallback"><span>{book.title}</span></div>}
                </div>
      ...
      {books.length === 0 && (
        <div className="empty-state"><strong>Your library is empty.</strong><span>Add your first book to get started.</span><Link href="/add" className="btn primary" style={{ marginTop: 12 }}>Add your first book</Link></div>
      )}

      {recent.length > 0 && (
        <section className="section" style={{ marginTop: 42 }}>
          <div className="section-head"><h2>Recently added</h2><Link href="/library" className="sub">View library →</Link></div>
          <div className="book-grid">
            {recent.map((book) => (
              <Link href={`/book/${book.id}`} className="book-card" key={book.id}>
                <div className="book-cover">{book.cover ? <OptimizedCover src={book.cover} /> : <div className="cover-fallback"><span>{book.title}</span></div>}</div>
